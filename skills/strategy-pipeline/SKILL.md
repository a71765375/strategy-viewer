...
参照 `html-strategy-processor` skill 的格式执行：

1. 创建 `docs/{描述性策略名}.md` — 结构：策略概述 → 附件 → 因子说明 → 代码 → 总结
2. 重命名 HTML 文件为简洁名（去掉任务标记和收益数字）
3. 更新 `htmls/README.md` 索引表（追加新行）
4. 更新 `notes/good_factor.md`（因子焦点摘要）
